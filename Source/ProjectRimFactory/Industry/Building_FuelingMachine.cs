...
namespace ProjectRimFactory.Industry
{
    public class Building_FuelingMachine : Building
    {
        ...
        protected IEnumerable<Thing> AllThingsForFueling
        {
            get
            {
                ...
                {
                    foreach (var item in GatherThingsUtility.AllThingsInCellForUse(cell, Map, false))
                    {
                        yield return item;
                    }
                }
        ...
        public static void RegisterRefuelable(Type buildingType,
            Func<Building, object> objectThatNeedsFueling,
            Func<object, Thing, int> fuelTest,
            Action<object, Thing> refuelAction)
        {
            registeredRefuelables.Add(new RegisteredRefuelable(buildingType, objectThatNeedsFueling, fuelTest, refuelAction));
        }
    ...
    static class RegisterVanillaMortarsAsRefuelable
    {
        static RegisterVanillaMortarsAsRefuelable()
        {
            ...
            //                               Action< object, Thing > refuelAction);
            refueler.GetMethod("RegisterRefuelable", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public)
                .Invoke(
                    null,
                    new object[]
                    {
