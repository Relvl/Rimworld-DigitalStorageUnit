...
namespace ProjectRimFactory.AutoMachineTool
{
    public class PlaceWorker_Conveyor : PlaceWorker
    {
        public override AcceptanceReport AllowsPlacing(BuildableDef checkingDef, IntVec3 loc, Rot4 rot, Map map, Thing thingToIgnore = null, Thing thing = null)
        {
            ...
            // underground:
            if (typeof(Building_BeltConveyorUGConnector).IsAssignableFrom(def.thingClass) || defModExtConveyor?.underground == true)
            {
                ...
                foreach (var t in map.thingGrid.ThingsListAt(loc))
                {
                    ...
                    if (t is Building_BeltConveyorUGConnector // other UGConnectors
                        ...
                        ||
                        (t is Blueprint blue &&
                         blue.def.entityDefToBuild is ThingDef td &&
            ...
            if (checkingDef.placeWorkers?.Contains(typeof(PlaceWorker_WallEmbedded)) == true)
            {
                ...
                {
                    if (t == thingToIgnore || t == thing) continue;
                    if (t is IBeltConveyorLinkable blet && blet is Thing bla && bla.def.placeWorkers?.Contains(typeof(PlaceWorker_WallEmbedded)) == true)
                    {
                        return new AcceptanceReport("PRFBlockedBy".Translate(t.Label));
                    }
                    ...
                    {
                        if (typeof(Building_BeltConveyor).IsAssignableFrom(td.thingClass) && td.placeWorkers?.Contains(typeof(PlaceWorker_WallEmbedded)) == true)
                        {
                            return new AcceptanceReport("PRFBlockedBy".Translate(t.Label));
                        }
                    }
