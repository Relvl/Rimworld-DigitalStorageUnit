...
namespace ProjectRimFactory.AutoMachineTool
{
    class Building_BeltConveyorUGConnector : Building_BeltConveyor, IBeltConveyorLinkable
    {
        ...
        {
            if (!ToUnderground || (IsStuck && WorkLeft < 0.05f) || (!IsStuck && WorkLeft > 0.85f))
            {
                base.DrawCarried();
            }
        }
        ...
        {
            if ((queryLevel == ConveyorLevel.Ground && def.GetModExtension<ModExtension_Conveyor>()?.toUnderground == true) ||
                (queryLevel == ConveyorLevel.Underground && def.GetModExtension<ModExtension_Conveyor>()?.toUnderground != true))
                return (defRotation != queryRotation);
            return false;
        }
    }
