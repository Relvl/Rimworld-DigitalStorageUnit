...
namespace ProjectRimFactory
{
    public class PRFGameComponent : GameComponent
    {
        ...
        public PRFGameComponent(Game game)
        {
            ...
            var data = (List<BackCompatibilityConverter>)SAL3.ReflectionUtility.BackCompatibility_conversionChain.GetValue(null);
        ...
        public bool TryAddSpecialSculpture(Thing item, SpecialSculpture specialSculpture = null, bool verifyProvidedSculpture = true)
        {
            ...
            else
            {
                ...
                foreach (var ss in ProjectRimFactory_ModComponent.availableSpecialSculptures.Where(s => (s.limitToDefs?.Contains(item.def) != false)).InRandomOrder())
                {
                    ...
                    if (inGameWithSameId.currentInstances == null || inGameWithSameId.currentInstances.Count(a => a != item) < inGameWithSameId.maxNumberCopies)
                    {
                        specialSculpture = ss;
                        break;
                    }
            ...
            var alreadyRecordedSpecialSculpture = specialScupltures.FirstOrDefault(s => s.id == specialSculpture.id);
        ...
        public static bool TryMakeProductSpecialScupture(List<Thing> products,
        ...
        public override void LoadedGame()
        {
            ...
            foreach (var queue in AssemblerQueue)
            {
                Debug.Message(Debug.Flag.AssemblerQueue, "" + queue + "  - " + AssemblerQueue[AssemblerQueue.IndexOf(queue)].Map);
                var building = queue as Building;
                Debug.Message(Debug.Flag.AssemblerQueue, "" + building.Map + " " + building.Position + "  --- ");
                foreach (var thing in AssemblerQueue[AssemblerQueue.IndexOf(queue)].Map.thingGrid.ThingsAt(building.Position))
                {
