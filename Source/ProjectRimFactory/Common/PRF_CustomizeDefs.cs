...
namespace ProjectRimFactory.Common
{
    ...
    class PRF_CustomizeDefs
    {
        ...
        {
            string[] ExcludeList_Primary =
            {
                "PRF_DeepQuarry_mkI", "PRF_DeepQuarry_mkII", "PRF_DeepQuarry_mkIII", "PRF_BillTypeMiner_I", "PRF_AutoCrafterSimple", "PRF_AutoCrafter", "PRF_TheArtMachine",
                "PRF_TheArtMaster", "PRF_OverclockedAutoAssembler", "PRF_OverclockedAutoAssemblerII", "PRF_SALAutoCooker", "PRF_SALAutoMinerI", "PRF_GodlyCrafter",
                "PRF_StoneWorks", "PRF_Recycler", "PRF_MrTsArtMachine", "PRF_MetalRefinery", "PRF_PartAssembler", "PRF_DroneCultivator_I", "PRF_DroneCultivator_II",
                "PRF_DroneCultivator_II_sun", "PRF_DroneCultivator_III", "PRF_OldTypeCultivator_I", "PRF_OldTypeCultivator_Sun", "PRF_OldTypeCultivator_Xl", "PRF_SelfCookerI",
                "PRF_SelfCookerII", "PRF_SelfCookerIII", "PRF_MeatGrinder", "PRF_FermentingBarrel", "PRF_Slaughterhouse", "PRF_AssemblerGroup", "PRF_RecipeDatabase",
                "PRF_Shearer", "PRF_Milker", "PRF_GenericAnimalHarvester", "PRF_GenericAnimalHarvester_II", "PRF_Sprinkler_I", "PRF_Sprinkler_II", "PRF_MineShaft",
                "PRF_MiniHelper", "PRF_MiniDroneColumn", "PRF_TypeOneAssembler_I", "PRF_TypeTwoAssembler_I", "PRF_TypeTwoAssembler_II", "PRF_TypeTwoAssembler_III",
                "PRF_FurnaceI", "PRF_FurnaceII", "PRF_FurnaceIII", "PRF_SelfPrepper", "PRF_Factory_Supplier", "PRF_ResearchTerminal", "TableRoboticMachining", "PRF_4k_Battery",
                "PRF_16k_Battery", "PRF_64k_Battery", "PRF_256k_Battery"
            };
            string[] ExcludeList_Signs =
            {
                "PRF_FloorLampArrow", "PRF_RedFloorLampArrow", "PRF_GreenFloorLampArrow", "PRF_FloorLampX", "PRF_FloorInput", "PRF_FloorOutput", "PRF_IconClothes",
                "PRF_IconSkull", "PRF_IconToxic", "PRF_IconPower", "PRF_IconGears", "PRF_IconGun", "PRF_IconGasmask", "PRF_IconFire", "PRF_IconCold", "PRF_IconDanger",
            ...
            string[] ExcludeList_Research =
            {
                "PRF_AutomaticFarmingI", "PRF_AutomaticFarmingII", "PRF_AutomaticFarmingIII", "PRF_BasicDrones", "PRF_ImprovedDrones", "PRF_AdvancedDrones",
                "PRF_AutonomousMining", "PRF_AutonomousMiningII", "PRF_AutonomousMiningIII", "PRF_SALResearchI", "PRF_SALResearchII", "PRF_SALResearchIII", "PRF_SALResearchIV",
                "PRF_SALResearchV", "PRF_SALResearchVII", "PRF_SALResearchVI", "PRF_SALResearchVIII", "PRF_SALGodlyCrafting", "PRF_EnhancedBatteries", "PRF_LargeBatteries",
                "PRF_VeryLargeBatteries", "PRF_UniversalAutocrafting", "PRF_SelfCorrectingAssemblers", "PRF_SelfCorrectingAssemblersII", "PRF_MetalRefining",
                "PRF_AnimalStations", "PRF_AnimalStationsII", "PRF_AnimalStationsIII", "PRF_SelfCooking", "PRF_SelfCookingII", "PRF_MachineLearning", "PRF_MagneticTape",
                "PRF_CoreTierO", "PRF_CoreTierI", "PRF_CoreTierII", "PRF_CoreTierIII", "PRF_CoreTierIV"
            };
            string[] ExcludeListTrader = { "PRF_Factory_Supplier" };

            //Components are items that are used in the creation of other items
            string[] RemoveList_Components =
            {
                "PRF_RoboticArm", "PRF_ElectronicChip_I", "PRF_ElectronicChip_II", "PRF_ElectronicChip_III", "PRF_DroneModule", "PRF_DataDisk", "PRF_MachineFrame_I",
                "PRF_MachineFrame_II", "PRF_MachineFrame_III"
            };

            if (remove)
            {
                var toRemove = ExcludeList_Primary.ToList();
            ...
            else
            {
                ...
                {
                    DefDatabase<ResearchProjectDef>.AllDefsListForReading[DefDatabase<ResearchProjectDef>.AllDefsListForReading.FirstIndexOf(c => c.defName == def.defName)]
                        .prerequisites = ProjectRimFactory_ModSettings.defTracker.GetDefaultValue<List<ResearchProjectDef>>(def.defName, "prerequisites");
                }
                ...
                {
                    DefDatabase<ResearchProjectDef>.AllDefsListForReading[DefDatabase<ResearchProjectDef>.AllDefsListForReading.FirstIndexOf(c => c.defName == def.defName)]
                        .requiredResearchFacilities = ProjectRimFactory_ModSettings.defTracker.GetDefaultValue<List<ThingDef>>(def.defName, "requiredResearchFacilities");
                }
                ...
                {
                    DefDatabase<ThingDef>.AllDefsListForReading[DefDatabase<ThingDef>.AllDefsListForReading.FirstIndexOf(c => c.defName == def.defName)].costList =
                        ProjectRimFactory_ModSettings.defTracker.GetDefaultValue<List<ThingDefCountClass>>(def.defName, "costList");
                }
        ...
        {
            var researchProjectsPre = DefDatabase<ResearchProjectDef>.AllDefsListForReading.Where(d => d?.prerequisites?.Where(c => defNames.Contains(c.defName)).Any() ?? false)
                .ToList();
            ...
            var researchProjectsFacilities = DefDatabase<ResearchProjectDef>.AllDefsListForReading
                .Where(d => d.requiredResearchFacilities?.Any(f => f.defName == "PRF_ResearchTerminal") ?? false)
                .ToList();
        ...
        {
            var checkRecipeLable = new Regex(@"^.+? x\d+$", RegexOptions.Compiled);

            var replacementDict = new Dictionary<ThingDef, List<IngredientCount>>();

            var recipeDefs2 = DefDatabase<RecipeDef>.AllDefsListForReading.Where(d => d.products.Select(s => s.thingDef).ToList().Where(c => defNames.Contains(c.defName)).Any())
                .ToList();
        ...
        public static void ArchitectMenu_ClearCache()
        {
            ...
            typeof(MainTabWindow_Architect).GetMethod("CacheDesPanels", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
