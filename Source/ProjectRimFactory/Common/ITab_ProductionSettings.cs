...
namespace ProjectRimFactory.Common
{
    ...
    class ITab_ProductionSettings : ITab
    {
        ...
        public static void RegisterSetting(Func<Thing, bool> showResultTest, Func<Thing, float> extraHeightRequest, Action<Thing, Listing_Standard> windowContents)
        {
        ...
        public override bool IsVisible
        {
            get
            {
                return showITabTests.FirstOrDefault(t => (t != null && t(SelThing))) != null ||
                       ShowProductLimt ||
                       ShowOutputToEntireStockpile ||
                       ShowObeysStorageFilter ||
                       ShowAdditionalSettings ||
                       ShowAreaSelectButton ||
                       ShowForbidOnPlacingSetting;
            }
        }

        bool ShowProductLimt => Machine != null && !Machine.ProductLimitationDisable;

        bool ShowOutputToEntireStockpile =>
            (PRFB != null &&
        ...
        bool ShowObeysStorageFilter =>
            (PRFB != null && (PRFB.SettingsOptions & PRFBSetting.optionObeysStorageFilters) > 0) &&
            !(PRFB is IBeltConveyorLinkable belt && !belt.CanSendToLevel(ConveyorLevel.Ground));
        ...
        protected override void UpdateSize()
        {
            ...
            for (var i = 0; i < showITabTests.Count; i++)
            {
            ...
            var maxHeight = 900f;
            var minHeight = 70f; // if this starts too large, the window will be too high
            var inspectWindowHeight = 268f; // Note: at least one mod makes this larger - this may not be enough.
        ...
        protected override void FillTab()
        {
            ...
            if (ShowObeysStorageFilter)
            {
                ...
                list.CheckboxLabeled("PRF.Common.ObeysStorageFilters".Translate(), ref tmpB, "PRF.Common.ObeysStorageFiltersDesc".Translate());
            ...
            {
                var tmpB = pRF_Building.ForbidOnPlacingDefault;
                list.CheckboxLabeled("PRF.Common.ForbidOnPlacingDefault".Translate(), ref tmpB, "PRF.Common.ForbidOnPlacingDefaultDesc".Translate());
            ...
            if (Machine != null && !Machine.ProductLimitationDisable)
            {
                ...
                if (Widgets.ButtonText(rect.RightHalf(), Machine.TargetSlotGroup?.parent?.SlotYielderLabel() ?? "PRF.AutoMachineTool.EntierMap".Translate()))
                {
                    Find.WindowStack.Add(
                        new FloatMenu(
                            groups.Select(g => new FloatMenuOption(g.parent.SlotYielderLabel(), () => Machine.TargetSlotGroup = g))
                                .ToList()
            ...
            if (ShowRangeTypeSelectorButton)
            {
                ...
                {
                    Find.WindowStack.Add(
                        new FloatMenu(compPropertiesPowerWork.rangeTypes.Select(d => new FloatMenuOption(d.ToText(), () => compPropertiesPowerWork.rangeCells = d)).ToList())
                    );
                }
