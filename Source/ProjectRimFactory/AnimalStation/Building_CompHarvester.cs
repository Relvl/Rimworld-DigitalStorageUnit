...
namespace ProjectRimFactory.AnimalStation
{
    public abstract class Building_CompHarvester : Building_Storage, IPowerSupplyMachineHolder
    {
        ...
        public IEnumerable<IntVec3> ScannerCells => GetComp<CompPowerWorkSetting>()?.GetRangeCells() ?? GenAdj.OccupiedRect(this).ExpandedBy(1).Cells;
        ...
        public override void TickRare()
        {
            ...
            foreach (var p in (from c in ScannerCells from p in c.GetThingList(Map).OfType<Pawn>() where p.Faction == Faction.OfPlayer select p).ToList())
            {
                foreach (var comp in (from comp in p.AllComps.OfType<CompHasGatherableBodyResource>() where CompValidator(comp) select comp).ToList())
                {
                    ...
                    if (amount != 0)
                    {
                        ...
                        while (amount > 0)
                        {
                            ...
                            amount -= num;
                            var thing = ThingMaker.MakeThing(resource, null);
                            thing.stackCount = num;
                            GenPlace.TryPlaceThing(thing, GetComp<CompOutputAdjustable>()?.CurrentCell ?? p.Position, p.Map, ThingPlaceMode.Near, null);
                        }
