...
namespace ProjectRimFactory.SAL3.Things.Assemblers
{
    public abstract class Building_ProgrammableAssembler : Building_DynamicBillGiver, IPowerSupplyMachineHolder, IAssemblerQueue
    {
        ...
        public override IEnumerable<Gizmo> GetGizmos()
        {
            ...
            if (Prefs.DevMode)
            {
                yield return new Command_Action() { defaultLabel = "DEBUG: Debug actions", action = () => Find.WindowStack.Add(new FloatMenu(GetDebugOptions().ToList())) };
            }
        ...
        {
            var stockpileCells = IngredientStackCells.Where(c => c != compOutputAdjustable.CurrentCell && !Map.thingGrid.ThingsListAt(c).Any(t => !t.def.CanOverlapZones));
            if (stockpileCells.Count() > 0)
            {
                var stockpileZone = new Designator_ZoneAddStockpile_Resources();
        ...
        protected virtual IEnumerable<FloatMenuOption> GetDebugOptions()
        {
            ...
            yield return new FloatMenuOption(
                "View all items available for input",
        ...
        public override void SpawnSetup(Map map, bool respawningAfterLoad)
        {
            ...
            buildingPawn?.skills.skills.ForEach(
                s =>
                {
                    s.Level = extension_Skills.GetExtendedSkillLevel(s.def, typeof(Building_ProgrammableAssembler));
            ...
            prf_gamecomp = Current.Game.GetComponent<PRFGameComponent>();
        ...
        protected virtual bool Active => compPowerTrader?.PowerOn != false && compRefuelable?.HasFuel != false && compFlick?.SwitchIsOn != false;

        protected IEnumerable<Thing> AllAccessibleThings =>
            from t in AllThingsInArea where (AllowForbidden || !t.IsForbidden(Faction)) where (!Map.reservationManager.AllReservedThings().Contains(t)) select t;
        ...
        /*=> from c in IngredientStackCells
                                                        ...
                                                        select t;*/
        protected IEnumerable<Bill> AllBillsShouldDoNow => from b in billStack.Bills where b.ShouldDoNow() select b;

        public override Thing GetThingBy(Func<Thing, bool> optionalValidator = null)
        {
            foreach (var t in thingQueue)
            {
                if (optionalValidator == null || optionalValidator(t))
                {
                    thingQueue.Remove(t);
                    return t;
                }
        ...
        public override void Tick()
        {
            ...
            {
                if (thingQueue.Count > 0 && PlaceThingUtility.PRFTryPlaceThing(this, thingQueue[0], compOutputAdjustable.CurrentCell, Map))
                {
            ...
            if (currentBillReport != null && Active)
            {
                ...
                if (effecter == null)
                {
                    effecter = (ext == null ? currentBillReport.bill.recipe?.effectWorking : ext.GetEffecter(currentBillReport.bill.recipe))?.Spawn();
                }

                if (sound == null)
                {
                    sound = (ext == null ? currentBillReport.bill.recipe?.soundWorking : ext.GetSound(currentBillReport.bill.recipe))?.TrySpawnSustainer(this);
                }
        ...
        protected BillReport TryGetNextBill()
        {
            ...
            foreach (var b in allBills)
            {
                ...
                var allAccessibleAllowedThings = allThings.Where(x => b.IsFixedOrAllowedIngredient(x)).ToList();
        ...
        bool TryFindBestBillIngredientsInSet(List<Thing> accessibleThings, Bill b, List<ThingCount> chosen)
        {
            ...
            var missing = new List<IngredientCount>(); // Needed for 1.4
        ...
        protected virtual void ProduceItems()
        {
            ...
            var products = GenRecipe.MakeRecipeProducts(
                currentBillReport.bill.recipe,
                buildingPawn,
            ...
            for (var i = 0; i < currentBillReport.selected.Count; i++)
            {
                if (currentBillReport.selected[i] is Corpse c)
                {
                    ...
                    {
                        var things = c.InnerPawn.inventory.innerContainer.ToList();
        ...
        public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            for (var i = thingQueue.Count - 1; i >= 0; i--)
            {
                ...
                PlaceThingUtility.PRFTryPlaceThing(this, thingQueue[i], Position, Map, true);
            }
        ...
        [Unsaved] private Effecter effecter = null;
        [Unsaved] private Sustainer sound = null;
